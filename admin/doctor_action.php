...
if (isset($_POST["action"])) {
  if ($_POST["action"] == 'fetch') {
    ...
    foreach ($result as $row) {
      ...
      if ($row["doctor_status"] == 'Active') {
        ...
        $status = '<button type="button" name="status_button" class="btn btn-danger btn-sm m-1 status_button" data-id="' . $row["doctor_id"] . '" data-status="' . $row["doctor_status"] . '">Inactive</button>
  ...
  if ($_POST["action"] == 'change_status') {
    ...
    if (isset($_POST['hidden_doctor_status'])) {
      ...
      $object->query = "
		UPDATE doctor_table 
		SET doctor_status = :doctor_status, inactivity_reason = :reason 
		WHERE doctor_id = '" . $_POST["doctor_hidden_id"] . "'
		";
    } else {
      $data = array(
