...
if (isset($_POST["action"])) {
  if ($_POST["action"] == 'fetch') {
    ...
    if ($_SESSION['type'] == 'Admin') {
      ...
      $order_column = array(
        ...
        'doctor_schedule_table.average_consulting_time'
      );
      $main_query = "
			SELECT *, doctor_schedule_table.inactivity_reason as 'reason' FROM doctor_schedule_table 
    ...
    foreach ($result as $row) {
      ...
      if ($row["doctor_schedule_status"] == 'Active') {
        ...
        if ($_SESSION["type"] == 'Admin') {
          $status = '<button type="button" name="status_button" class="btn btn-danger btn-sm m-1 status_button" data-id="' . $row["doctor_schedule_id"] . '" data-status="' . $row["doctor_schedule_status"] . '">Inactive</button>
        <button type="button" name="reason_button" class="btn btn-dark btn-sm m-1 reason_button" data-reason="' . $row["reason"] . '">Reason</button>';
        } else {
          $status = '<button type="button" name="status_button" class="btn btn-danger btn-sm m-1 status_button" data-id="' . $row["doctor_schedule_id"] . '" data-status="' . $row["doctor_schedule_status"] . '">Inactive</button>
          <button type="button" name="reason_button" class="btn btn-dark btn-sm m-1 reason_button" data-reason="' . $row["inactivity_reason"] . '">Reason</button>';
        }
  ...
  if ($_POST["action"] == 'change_status') {
    ...
    if (isset($_POST['hidden_doctor_status'])) {
      $data = array(
        ':doctor_status' => "Inactive",
        ':reason' => $_POST['reason_for_inactivity'],
      );
      $object->query = "
		UPDATE doctor_schedule_table 
		SET doctor_schedule_status = :doctor_status, inactivity_reason = :reason 
		WHERE doctor_schedule_id = '" . $_POST["doctor_hidden_id"] . "'
		";
    } else {
      $data = array(
      ...
      $object->query = "
		UPDATE doctor_schedule_table 
		SET doctor_schedule_status = :doctor_status,
    ...
    $object->execute($data);
    if (isset($_POST['hidden_doctor_status'])) {
      echo '<div class="alert alert-success">Class Status change to ' . $_POST['hidden_doctor_status'] . '</div>';
    } else {
      echo '<div class="alert alert-success">Class Status change to ' . $_POST['status'] . '</div>';
    }
  }
